...
// TODO: This file needs to be integrated into the new SparseStorage container.
#if 0
...
public:
    gpu_mesh_manger();
    void Deinit(struct GpuFrameCache* FrameCache);

    gpu_mesh_id AcquireMesh();                                          // Acquire a new id for a mesh. Guarenteed to be unique.
    void ReleaseMesh(gpu_mesh_id Id, struct GpuFrameCache* FrameCache);   // Release a mesh id. The data for the mesh is removed and unallocated.
